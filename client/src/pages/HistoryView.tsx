          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="hidden sm:block">
                ...
                <p className="text-xs" style={{ color: "rgba(var(--zeglam-text-rgb),0.4)" }}>Auto-fechamentos</p>
              </div>
            </div>
            <Button variant="ghost" size="sm" onClick={() => setLocation('/admin')}
              className="text-xs" style={{ color: "rgba(var(--zeglam-text-rgb),0.5)" }}>
              ← Voltar
            </Button>
          </div>
        </div>
        ...
        <div className="rounded-xl overflow-hidden" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(184,160,96,0.15)" }}>
            ...
            <table className="w-full">
              <thead>
                <tr style={{ background: "rgba(10,22,40,0.8)", borderBottom: "1px solid rgba(184,160,96,0.15)" }}>
                  <th className="px-4 py-3 text-left text-xs font-medium" style={{ color: "rgba(var(--zeglam-text-rgb),0.5)" }}>ID</th>
                  <th className="px-4 py-3 text-left text-xs font-medium" style={{ color: "rgba(var(--zeglam-text-rgb),0.5)" }}>Link</th>
                  <th className="px-4 py-3 text-left text-xs font-medium" style={{ color: "rgba(var(--zeglam-text-rgb),0.5)" }}>Horário Programado</th>
                  <th className="px-4 py-3 text-left text-xs font-medium" style={{ color: "rgba(var(--zeglam-text-rgb),0.5)" }}>Data de Fechamento</th>
                </tr>
              </thead>
              <tbody>
                {isLoading ? (
                  <tr>
                    <td colSpan={4} className="px-4 py-8 text-center" style={{ color: "rgba(var(--zeglam-text-rgb),0.4)" }}>
                      Carregando...
                    </td>
                  </tr>
                  ...
                  historyData.map((item: any) => (
                    <tr key={item.id} style={{ borderBottom: "1px solid rgba(184,160,96,0.1)" }}>
                      <td className="px-4 py-3 text-sm" style={{ color: "rgba(var(--zeglam-text-rgb),0.7)" }}>{item.id}</td>
                      <td className="px-4 py-3 text-sm" style={{ color: "rgba(var(--zeglam-text-rgb),0.7)" }}>{item.linkNome}</td>
                      <td className="px-4 py-3 text-sm" style={{ color: "rgba(var(--zeglam-text-rgb),0.7)" }}>{item.scheduledCloseTime}</td>
                      <td className="px-4 py-3 text-sm" style={{ color: "rgba(var(--zeglam-text-rgb),0.7)" }}>{new Date(item.closedAt).toLocaleString('pt-BR')}</td>
                    </tr>
                  ))
                )}
              </tbody>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-4 py-3"
              ...
              <p className="text-xs" style={{ color: "rgba(var(--zeglam-text-rgb),0.4)" }}>Página {page + 1} de {totalPages}</p>
